...
# 나머지 변수는 고정값으로 설정
make_name="Make_512_vla_data_frame_${frame_num}"
only_check=false


# Task 배열 정의
tasks=('pickup_object' 'reorient_object' 'pour_water' 'open_drawer')
...
if [ "$only_check" = true ]; then
    output_root_base="/root/arnold/VLA_512_DATA/${make_name}_only_check"
else
    output_root_base="/root/arnold/VLA_512_DATA/${make_name}_NEW"
fi
...
for task in "${tasks[@]}"; do
    ...
    if [ -d "$task_dir" ]; then
        ...
        for i in {1..1}; do
            ...
            /isaac-sim/python.sh "/root/arnold/make_512_vla_data_0.py" \
                task="$task" \
                use_gt="[1,1]" \
                visualize=0 \
                record=False \
