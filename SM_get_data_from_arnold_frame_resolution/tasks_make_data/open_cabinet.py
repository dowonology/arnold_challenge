...
from utils.env import position_reached, rotation_reached, get_pre_grasp_action, action_interpolation, get_obs_make_data
...
class OpenCabinet(BaseTask):
    ...
    def reset(self, robot_parameters, 
        ...
        obs = super().reset(
            ...
            sensor_resolution = scene_resolution
        )
        # used for max number of steps (grasp, raise up)
    ...
    def load_object(self):
       ...
       # pdb.set_trace()
        ...
        # TODO
        # use Isaac Sim default collision for handle processing. This is not needed if we apply Su Hao's method first
        ...
        # Save as USDZ file
        self.save_to_usdz(object_prim_path)

    def save_to_usdz(self, root_prim_path: str, file_name: str = "saved_scene.usdz"):
        """
        ...
        if stage:
            try:
                # 저장할 파일의 경로 지정
                output_path = os.path.join(os.getcwd(), file_name)  # 현재 작업 디렉토리에 저장
                
                # 루트 레이어를 USDZ 파일로 내보내기
                usdz_path = os.path.join(os.getcwd(), file_name)
                # Use the USD API to create a USDZ file
                usd_exporter = UsdUtils.StageCache.ExportToUsdz(stage, root_prim_path, usdz_path)
                
                # Check if the USDZ export was successful
                if usd_exporter:
                    print(f"USDZ 파일로 저장 완료: {usdz_path}")
                else:
                    print(f"USDZ 파일 저장 실패")

            except Exception as e:
                print(f"USDZ 파일 저장 실패: {e}")



    def step(self, act_pos, act_rot, render, use_gt, file_path:str=None, franka = None , cspace_controller = None, make_npz_data = None, target_frame_num=None):
        """
        ...
        while self.current_stage < self.end_stage:
            if self.time_step % int(target_frame_num) == 0:
                diff_data = {'diff' : self.checker.get_diff()}
                make_npz_data.append(copy.deepcopy({**self.render(), **get_obs_make_data(franka=franka, cspace_controller=cspace_controller), **diff_data}))
            ...
            simulation_context.step(render=render)
            self.time_step += 1
        print(f"최종 진입 Stage:{self.current_stage}/{self.num_stages}\n최종 소요 시간:{self.time_step}/2400\n")    #Stage

        with open(file_path, 'a') as file:
            file.write(f"최종 진입 Stage:{self.current_stage}/{self.num_stages}\n")    #Stage
            file.write(f"최종 소요 시간:{self.time_step}/2400\n")    #소요시간
        if self.current_stage == self.num_stages:
            ...
            #     # make NPZ data (1, 2 stage)
            #     diff_data = {'diff' : self.checker.get_diff()}
            #     make_npz_data.append({**self.render(), **get_obs_make_data(franka=franka, cspace_controller=cspace_controller), **diff_data})
