...
simulation_app, simulation_context, _ = get_simulation(headless=True, gpu_id=3)
...
def main(cfg):
    ...
    offset = cfg.offset_bound       # 저자가 설정한 바운더리
    use_gt = cfg.use_gt             # 로봇 동작의 정답(grasp, manipulate) [2] -> [1, 0], [0, 1] => 30% (성능 검증용)
    if not (use_gt[0] and use_gt[1]):
        agent, lang_embed_cache = load_agent(cfg, device=device) # 학습한 agent(로봇) 모델 부르기기
    ...
    if use_gt[0]:
        if use_gt[1]:
            eval_setting = '2gt' # grasp + manipulate 사용
        else:
            eval_setting = '1gt' # grasp 만 사용
    else:
        eval_setting = '0gt'
    ...
    for task in task_list:
        ...
        for eval_split in cfg.eval_splits:
            ...
            if os.path.exists(os.path.join(cfg.data_root, task, eval_split)):
                ...
                data, fnames = load_data(data_path=os.path.join(cfg.data_root, task, eval_split)) # .npz 포멧 ()
            else:
            ...
            while len(data) > 0:
                ...
                fname = fnames.pop(0)
                gt_frames = anno['gt']
                robot_base = gt_frames[0]['robot_base'] # 초기 로봇의 밑동 위치
                ...
                env, object_parameters, robot_parameters, scene_parameters = load_task(cfg.asset_root, npz=anno, cfg=cfg) # 시뮬레이션 환경 불러오기 + 환경 세팅

                # [디버깅] env 내부에 파일 이름 저장
                env.current_data_name = fname
                print("DEBUG: current_data_name →", env.current_data_name)
                ...
                try:
                    for i in range(2):
                        if use_gt[i]:
                            ...
                            ) # obs(현재 3d 환경), agent(내가 학습시킨 로봇 모델)
